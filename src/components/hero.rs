...
use crate::components::{
    button::{ButtonLink, Variant},
    container::Container,
};
use leptos::{component, server, server_fn::client, view, CollectView, IntoView, ServerFnError, Suspense};
...
static HERO_IMAGES: &[&[Image]] = &[
    &[
        Image {
            name: "Transistor",
            logo: "/images/logos/transistor.svg",
        },
        Image {
            name: "Tuple",
        ...
        Image {
            name: "StaticKit",
            logo: "/images/logos/statickit.svg",
        },
    ],
    &[
        Image {
...
pub type ApiResponse = Vec<SanityResponse>;

#[derive(Default, Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct SanityResponse {
    pub what_we_expect: Vec<String>,
    pub who_we_want: String,
    pub why_us: String,
...
#[server(GetJobData, "/api")]
pub async fn fetch_data() -> Result<ApiResponse, ServerFnError> {
    let client = reqwest::Client::new();
    let url = format!("https://ly6kxhrq.api.sanity.io/v2022-03-07/data/query/production?query=*%5B_type+%3D%3D+%27jobPosting%27%5D");

    let response = client.get(&url).send().await.map_err(|e| JobError::FetchError(e))?;
...
pub fn Hero() -> impl IntoView {
    view! {
    <Suspense fallback=|| ()>
      <Container class=String::from("pb-16 pt-20 text-center lg:pt-32")>
        ...
        <p class="mx-auto mt-6 max-w-2xl text-lg tracking-tight text-slate-700">
          Most job seeking software is accurate, but hard to use. We make the opposite trade-off, and help
          you gain new employees.
        </p>
        <div class="mt-10 flex justify-center gap-x-6">
          <ButtonLink href=String::from("/register")>Get 6 months free</ButtonLink>
          <ButtonLink href=String::from("https://youtu.be/sxjgL64czRY") variant=Variant::Outline>
